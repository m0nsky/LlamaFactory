...
def get_train_args(args: dict[str, Any] | list[str] | None = None) -> _TRAIN_CLS:
    ...
    # Auto-enable packing for PT stage if not explicitly set, then apply the packing decrement of
    # `cutoff_len` — but only for SFT, which is the only stage that needs it.
    #
    # Why the -1 exists at all: `PackedSupervisedDatasetProcessor` emits sequences of
    # `cutoff_len + 1` tokens (it pads to that length and asserts it), so the decrement is what
    # makes a requested 4096 actually come out as 4096. PT packing slices at exactly
    # `block_size = cutoff_len` and has no such +1, so for PT the decrement cancels nothing and
    # blocks came out one token short — 4095 for a configured 4096.
    #
    # This has to live here rather than in `DataArguments.__post_init__`, which runs while parsing
    # the YAML and has no access to `finetuning_args.stage`. Doing it here means both the explicit
    # `packing: true` path and the auto-enabled PT path go through the same single adjustment,
    # instead of __post_init__ and this function each applying their own.
    #
    ...
    if data_args.packing and finetuning_args.stage == "sft":
        data_args.cutoff_len -= 1  # see the comment above: SFT packing emits cutoff_len + 1 tokens
